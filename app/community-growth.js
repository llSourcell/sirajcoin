const secp256k1 = require('secp256k1') // eslint-disable-line import/no-extraneous-dependencies
...
module.exports = (opts) => {
  ...
  const oraclePubkey = Buffer.from(opts.oraclePubkey, 'hex')
  ...
  return {
    ...
    onInput(input, tx, state) {
      ...
      const grantAmount = tx.to[0].amount

      // second output must pay a percentage of grant to community treasury
      const treasuryPayout = tx.to[1]
      const expectedTreasuryAmount = Math.floor((grantAmount * treasuryPercent) / 100)
      ...
      const foundersPayout = tx.to[2]
      const expectedFoundersAmount = Math.floor((grantAmount * foundersPercent) / 100)
